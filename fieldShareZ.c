...
void MPI_Transfer1F_Zplus(double complex ***f1,int harmony,int N,int fromI,int toI)
{
    ...
    else if(myrank%2==1 && myrank>0)
    {
       MPI_Recv(recv,num,MPI_DOUBLE,myrank-1,myrank-1, MPI_COMM_WORLD,&status);  
       start=0; 
       for(h=0; h<harmony; h++)  {
         for(n=0; n<N; n++) {
           realV=recv[start+2*n+0];
	        imagV=recv[start+2*n+1];
