...
void saveFieldHDF(Domain *D,int iteration)
{
   ...
   for(h=0; h<D->numHarmony; h++) {
     ...
     saveFieldComp(D->U,fileName,dataName,h,sliceN,N,subSliceN,D->saveStep,D->minI);
     MPI_Barrier(MPI_COMM_WORLD);
...
//   if(D->dimension==1) area=2.0*M_PI*LL->sigX*LL->sigY;
   else                area=D->dx*D->dy;
...
//   coef2=coef*coef*eps0*velocityC*area;
   ...
   if(myrank==0) { 
     ...
     saveDoubleMeta(fileName,"powerCoef",&coef2,1);
