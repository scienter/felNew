...
void parameterSetting(Domain *D,char *input)
{
   ...
   B0=D->K0*sqrt(2.0)/eCharge*eMass*velocityC*D->ku;	
   D->KRef=D->K0;
   LL = D->loadList;
   D->gamma0=LL->energy/mc2+1;	
     ...
     if(FindParameters("Chicane",rank,"selfseed_ONOFF",input,str)) Chi->selfSeedON=whatONOFF(str);
	  ...
	  if(Chi->selfSeedON==ON) {
       ...
       if(FindParameters("Chicane",rank,"crystal_type",input,str)) whatCrystal(D->ks,Chi,str);
         ...
         if(FindParameters("Chicane",rank,"noise_ONOFF",input,str)) Chi->noiseONOFF=whatONOFF(str);
         else  { printf("In [Chicane], noise_ONOFF=? [ON or OFF].\n"); fail=1;  }
...
{
   char name[100], str[100];
   double unitLength,undLength,undStart,K0;
   int i,fail=0;
   int myrank, nTasks;
   MPI_Status status;
   ...
   if(FindParameters("Undulator",rank,"undulator_mode",input,str)) UL->undMode=whatUndType(str);
   else  UL->undMode=Linear;	
   ...
   if(UL->numbers>0) {
     ...
     if(FindParameters("Undulator",rank,"K0",input,str)) UL->K0[0]=atof(str)/sqrt(2.0);
     else  { printf("in [Undulator], K0=? [T]\n");  fail=1;   }
     ...
     for(i=1; i<UL->numbers; i++)  {
       ...
       UL->K0[i]=K0+i*UL->linTaper+i*i*UL->quadTaper;
     }
   } else ;	//end of if(UL->numbers>0)
...
int whatUndType(char *str)
{
   if(strstr(str,"Linear"))            return Linear;
   else if(strstr(str,"Helical"))       return Helical;
   else           return 0;
}
